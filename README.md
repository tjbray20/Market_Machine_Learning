...
## Methodology
I use 6 different model types throughout this project. 3 Classification Models and 3 Regression Models. The Regression Models are Elastic Net, Support Vector Regression, and Ridge Regression. The Classification Models are Logistic Regression, Decision Tree, and XGBoost. These models are compared against a baseline model of the stock continuing to move in the same direction it moved a majority of the time in the training set.

## Results
While no specific model type outperformed the baseline in the aggregate. There were many instances of individual models for specific companies outperforming the baseline. Biogen Inc had the top two models, SVR and Decision Tree, they outperformed the baseline by 8.2% and 6%, respectively. 
